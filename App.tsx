...
const App: React.FC = () => {
  const { user, loading, isActive, hasApiKey } = useAuth();
  ...
  useEffect(() => {
    if (user && isActive && hasApiKey) {
      ...
      const storedDate = localStorage.getItem('lovspeak_wisdom_date');
      const storedCount = parseInt(localStorage.getItem('lovspeak_wisdom_count') || '0');

      if (storedDate !== today) {
        localStorage.setItem('lovspeak_wisdom_date', today);
        localStorage.setItem('lovspeak_wisdom_count', '1');
        ...
        localStorage.setItem('lovspeak_wisdom_count', (storedCount + 1).toString());
        const randomQuote = ISLAMIC_QUOTES[Math.floor(Math.random() * ISLAMIC_QUOTES.length)];
        setCurrentQuote(randomQuote);
        setShowWisdomModal(true);
      }
      ...
      default:
        ...
        return (
          <div className="space-y-10 animate-fade-in">
            ...
            <div className="relative overflow-hidden rounded-3xl md:rounded-[2.5rem] bg-gradient-to-br from-lovelya-600 to-lovelya-500 shadow-xl p-5 md:p-10 text-white">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 md:gap-6">
                    <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-1.5 mb-1.5 md:mb-3">
                            <span className="bg-white/20 backdrop-blur-md px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[7px] md:text-[10px] font-black uppercase tracking-widest">
                                {currentLevel} Level
                            </span>
                            <span className="bg-yellow-400/20 backdrop-blur-md px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[7px] md:text-[10px] font-black uppercase tracking-widest text-yellow-200">
                                {userProfile?.xp || 0} XP
                            </span>
                        </div>
                        <h1 className="text-lg md:text-5xl font-black tracking-tight mb-1 md:mb-2 leading-tight">
                            Welcome back, <br className="md:hidden" />
                            <span className="text-white">
                                {userProfile?.name ? userProfile.name.split(' ')[0] : 'Lovelies'}
                            </span>
                        </h1>
                        <p className="text-lovelya-100 text-[9px] md:text-lg font-medium max-w-xl opacity-90">
                    ...
                    <div className="flex flex-row md:flex-row gap-2 w-full md:w-auto">
                        <button 
                            ...
                            className="flex-1 md:flex-none bg-white/20 hover:bg-white/30 backdrop-blur-md border border-white/40 text-white px-3 md:px-5 py-2 md:py-3 rounded-xl md:rounded-2xl font-bold transition active:scale-95 flex items-center justify-center gap-1.5 md:gap-2 text-[9px] md:text-sm"
                            title="Daily Wisdom"
                        >
                            <i className="fas fa-quote-left"></i> <span>Wisdom</span>
                        </button>
                        <button onClick={() => setView(AppView.ASSESSMENT)} className="flex-1 md:flex-none bg-white text-lovelya-600 px-3 md:px-6 py-2 md:py-3 rounded-xl md:rounded-2xl font-bold shadow-lg hover:shadow-xl transition active:scale-95 flex items-center justify-center gap-1.5 md:gap-2 text-[9px] md:text-sm">
                            <i className="fas fa-graduation-cap"></i> <span>Test</span>
                        </button>
                        <button onClick={openPlanModal} className="flex-1 md:flex-none bg-white/20 hover:bg-white/30 backdrop-blur-md border border-white/40 text-white px-3 md:px-5 py-2 md:py-3 rounded-xl md:rounded-2xl font-bold transition active:scale-95 flex items-center justify-center gap-1.5 md:gap-2 text-[9px] md:text-sm">
                            <i className="fas fa-sliders-h"></i> <span>Plan</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="pt-2 md:pt-4">
               <div className="flex items-center justify-between mb-4 md:mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-lg md:text-xl font-black text-gray-800 dark:text-white tracking-tight">Modul</h3>
                  </div>
                  <button 
                    onClick={() => setShowModules(!showModules)} 
                    className="flex items-center gap-2 px-3 py-1 md:px-4 md:py-1.5 bg-lovelya-50 dark:bg-lovelya-900/30 text-lovelya-600 dark:text-lovelya-400 rounded-full font-bold text-[10px] md:text-xs hover:bg-lovelya-100 dark:hover:bg-lovelya-900/50 transition-all active:scale-95"
                  >
                    <i className={`fas ${showModules ? 'fa-eye-slash' : 'fa-eye'}`}></i>
                    {showModules ? 'Hide' : 'Unhide'}
                  </button>
               </div>
               ...
               <AnimatePresence>
                 {showModules && (
                   ...
                   >
                     <div className="grid grid-cols-4 sm:grid-cols-4 gap-2 md:gap-4 pb-2">
                        {[
                          ...
                          {id: AppView.SHADOWING, icon: 'fa-wave-square', label: 'Shadowing', color: 'text-lovelya-600', bg: 'bg-lovelya-50 dark:bg-lovelya-900/20'},
                          {id: AppView.DIARY, icon: 'fa-book-bookmark', label: 'Diary', color: 'text-indigo-500', bg: 'bg-indigo-50 dark:bg-indigo-900/20'},
                          {id: AppView.CHAT, icon: 'fa-robot', label: 'AI Tutor', color: 'text-cyan-500', bg: 'bg-cyan-50 dark:bg-cyan-900/20'},
                          {id: AppView.TRANSLATE, icon: 'fa-language', label: 'Translate', color: 'text-lovelya-500', bg: 'bg-lovelya-50 dark:bg-lovelya-900/20'},
                        ].map(item => (
                            <button 
                              key={item.id}
                              onClick={() => setView(item.id as AppView)}
                              className="flex flex-col items-center justify-center p-2 md:p-4 bg-white dark:bg-gray-800 rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 hover:border-lovelya-300 hover:shadow-md transition group aspect-square"
                            >
                              <div className={`w-8 h-8 md:w-12 md:h-12 rounded-xl md:rounded-2xl ${item.bg} flex items-center justify-center text-sm md:text-xl ${item.color} mb-1 md:mb-2 group-hover:scale-110 transition-transform`}>
                                <i className={`fas ${item.icon}`}></i>
                              </div>
                              <span className="font-bold text-gray-600 dark:text-gray-300 text-[8px] md:text-[10px] text-center leading-tight">{item.label}</span>
                            </button>
                        ))}
                     </div>
                   </motion.div>
            ...
            <div className="block">
              {!plan ? (
                  <div className="text-center py-12 md:py-20 bg-white dark:bg-gray-800 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-sm px-6">
                      ...
                      <p className="text-xs md:text-sm text-gray-500 max-w-md mx-auto mb-8">Setup your personalized learning path to get daily tasks tailored to your needs.</p>
                      <button onClick={openPlanModal} className="bg-lovelya-600 text-white px-6 py-3 md:px-8 md:py-4 rounded-2xl font-bold text-base md:text-lg shadow-lg hover:bg-lovelya-700 transition active:scale-95">
                          Create My Plan
                      </button>
                  </div>
              ) : (
                  <div className="space-y-8">
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                          <div className="flex items-center gap-2">
                            <div className="w-1.5 h-6 bg-lovelya-600 rounded-full"></div>
                            <h3 className="text-xl md:text-2xl font-black text-gray-800 dark:text-white tracking-tight">Learning Schedule</h3>
                          </div>
                          <div className="flex flex-col items-end gap-2">
                              <div className="bg-gray-100 dark:bg-gray-700 p-1 rounded-2xl flex shrink-0 w-full sm:w-auto overflow-hidden">
                                  <button onClick={() => setDashboardView('today')} className={`flex-1 sm:flex-none px-3 py-1.5 md:px-4 md:py-2 text-[9px] md:text-[10px] font-black uppercase tracking-wider rounded-xl transition ${dashboardView === 'today' ? 'bg-white dark:bg-gray-600 text-lovelya-600 dark:text-white shadow-sm' : 'text-gray-500'}`}>Today</button>
                                  <button onClick={() => setDashboardView('week')} className={`flex-1 sm:flex-none px-3 py-1.5 md:px-4 md:py-2 text-[9px] md:text-[10px] font-black uppercase tracking-wider rounded-xl transition ${dashboardView === 'week' ? 'bg-white dark:bg-gray-600 text-lovelya-600 dark:text-white shadow-sm' : 'text-gray-500'}`}>7 Days</button>
                                  <button onClick={() => setDashboardView('month')} className={`flex-1 sm:flex-none px-3 py-1.5 md:px-4 md:py-2 text-[9px] md:text-[10px] font-black uppercase tracking-wider rounded-xl transition ${dashboardView === 'month' ? 'bg-white dark:bg-gray-600 text-lovelya-600 dark:text-white shadow-sm' : 'text-gray-500'}`}>30 Days</button>
                              </div>
                              
                              <button 
                                  onClick={handleAddToCalendar} 
                                  className="text-[9px] md:text-[10px] font-black text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 flex items-center gap-1 transition-colors px-2 py-1 uppercase tracking-widest"
                              >
                                  <i className="fas fa-sync-alt text-[8px]"></i> sync with google
                              </button>
                          </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                          ...
                          <div className="space-y-6">
                              <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] p-6 shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden group">
                                  ...
                                  <h3 className="font-black text-gray-800 dark:text-white mb-6 flex items-center gap-2 relative z-10 text-sm md:text-base">
                                      <i className="fas fa-bullseye text-lovelya-500"></i> Daily Goal
                                  </h3>
                                  <div className="relative w-24 h-24 md:w-36 md:h-36 lg:w-40 lg:h-40 mx-auto mb-4 md:mb-6 flex items-center justify-center z-10">
                                      ...
                                      <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-800 dark:text-white">
                                          <span className="text-lg md:text-3xl font-black">{progressPercent}%</span>
                                      </div>
                                  </div>
