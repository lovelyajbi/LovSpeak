...
const VocabularyModule: React.FC<ModuleProps> = ({ onComplete, onNavigate }) => {
  ...
  return (
    <div className="space-y-4 md:space-y-6 h-full flex flex-col">
      ...
      <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm p-3.5 md:p-5 rounded-2xl md:rounded-3xl shadow-sm border border-lovelya-100 dark:border-gray-700 relative mx-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-4">
            <div>
                <h2 className="text-lg md:text-2xl font-extrabold text-gray-800 dark:text-white flex items-center">
                    <i className="fas fa-book-reader text-lovelya-500 mr-2 md:mr-3"></i>
                    Vocabulary
                </h2>
                <p className="text-[9px] md:text-xs text-gray-500 dark:text-gray-400 mt-0.5 md:mt-1 uppercase tracking-widest font-bold">
                    {isSearching 
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
                <div className="relative flex-1 md:w-56">
                    <i className="fas fa-search absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 text-[9px] md:text-xs"></i>
                    <input 
                    type="text"
                    placeholder="Cari kata..."
                    value={filter}
                    onChange={e => { setFilter(e.target.value); setCurrentPage(1); }}
                    className="pl-8 pr-3 py-2 md:py-2 w-full rounded-lg md:rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 outline-none focus:ring-2 focus:ring-lovelya-300 transition-all text-[11px] md:text-sm"
                    />
                </div>
                
                <button 
                    onClick={openAddModal}
                    className="w-9 h-9 md:w-10 md:h-10 bg-lovelya-500 text-white rounded-lg md:rounded-xl shadow-lg shadow-lovelya-200/50 hover:bg-lovelya-600 transition flex items-center justify-center shrink-0"
                >
          ...
          <div className="animate-fade-in px-4">
             <div className="mb-4 md:mb-6">
                <button 
                  ...
                  className="w-full md:w-auto flex items-center justify-center p-3.5 md:p-5 bg-gradient-to-r from-red-50 to-pink-50 dark:from-red-900/20 dark:to-pink-900/20 rounded-2xl border border-red-100 dark:border-red-900 shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all duration-300 group"
                >
                  <div className="w-10 h-10 md:w-14 md:h-14 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center mr-3 md:mr-4 text-red-500 shadow-sm group-hover:scale-110 transition">
                    <i className="fas fa-heart text-lg md:text-2xl"></i>
                  </div>
                  <div className="text-left mr-4 md:mr-8">
                    <h3 className="font-bold text-gray-800 dark:text-gray-100 text-xs md:text-lg">Favorites</h3>
                    <p className="text-[9px] md:text-sm text-gray-500">{groupedVocab['Favorites']?.length || 0} kata</p>
                  </div>
                  <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-red-100 dark:bg-red-900/50 flex items-center justify-center text-red-400 group-hover:bg-red-500 group-hover:text-white transition ml-auto">
                 ...
                 return (
                   <div key={cat} className="relative group h-full">
                     <button 
                       ...
                       className="w-full flex flex-col items-center justify-center p-3.5 md:p-6 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-soft hover:border-lovelya-200 transition-all duration-300 text-center h-full relative overflow-hidden"
                     >
                       <div className="absolute top-0 right-0 p-2 opacity-5">
                          <i className={`fas ${categoryIcons[cat] || 'fa-folder'} text-3xl md:text-6xl`}></i>
                       </div>
                       <div className="w-10 h-10 md:w-16 md:h-16 bg-lovelya-50 dark:bg-gray-700 rounded-xl md:rounded-2xl flex items-center justify-center mb-2 md:mb-4 text-lg md:text-2xl group-hover:scale-110 transition duration-300 z-10 text-lovelya-500 shadow-inner">
                          <i className={`fas ${categoryIcons[cat] || 'fa-folder'}`}></i>
                       </div>
                       <h3 className="font-bold text-gray-800 dark:text-gray-200 text-[11px] md:text-sm mb-1 group-hover:text-lovelya-600 w-full z-10 line-clamp-1">
                         {cat}
                       </h3>
                       <span className="text-[8px] md:text-xs text-gray-400 bg-gray-50 dark:bg-gray-700 px-2 py-0.5 md:py-1 rounded-full mt-auto z-10 font-medium">
                         {count} kata
                       </span>
                     </button>
                   </div>
           ...
           <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden flex flex-col h-[calc(100vh-240px)] md:h-[calc(100vh-220px)] animate-slide-up mx-4">
              <div className="p-2 md:p-3 border-b border-gray-100 dark:border-gray-700 bg-lovelya-50/50 dark:bg-gray-900/50 flex items-center justify-between">
                   <div className="flex items-center gap-2 text-[9px] md:text-xs">
                     ...
                     <span className="font-bold text-lovelya-600 dark:text-lovelya-400 flex items-center gap-1 bg-white dark:bg-gray-700 px-2 py-1 rounded-lg border border-gray-100 dark:border-gray-600 shadow-sm">
              ...
              <div className="flex-1 overflow-auto custom-scrollbar">
                <table className="w-full text-left border-collapse">
                  <thead className="bg-gray-50 dark:bg-gray-900/50 sticky top-0 z-10 backdrop-blur-sm">
                    <tr className="text-gray-400 dark:text-gray-500 text-[7px] md:text-[10px] uppercase tracking-wider font-bold">
                      <th className="p-2 md:p-3 border-b border-gray-100 dark:border-gray-700 w-7 md:w-12 text-center">#</th>
                      <th className="p-2 md:p-3 border-b border-gray-100 dark:border-gray-700">English</th>
                      <th className="p-2 md:p-3 border-b border-gray-100 dark:border-gray-700">Indonesian</th>
                      <th className="p-2 md:p-3 border-b border-gray-100 dark:border-gray-700 text-center w-14 md:w-24">Aksi</th>
                    </tr>
                  </thead>
                    ...
                    {paginatedItems.map((item, idx) => {
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-lovelya-50/50 dark:hover:bg-gray-700/30 transition-colors group">
                          <td className="p-2 md:p-3 text-gray-400 text-[9px] font-mono text-center">{displayIdx}</td>
                          <td className="p-2 md:p-3">
                            <span className="font-bold text-gray-800 dark:text-gray-100 text-[11px] md:text-sm">{item.english}</span>
                          </td>
                          <td className="p-2 md:p-3">
                            <span className="text-gray-600 dark:text-gray-300 italic text-[10px] md:text-sm">{item.indonesian}</span>
                          </td>
                          <td className="p-2 md:p-3 text-center">
                            <div className="flex items-center justify-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                              <button 
                                onClick={() => playPronunciation(item.english)}
                                className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 text-lovelya-500 hover:bg-lovelya-500 hover:text-white transition flex items-center justify-center shadow-sm"
                                title="Dengarkan"
                              >
                                <i className="fas fa-volume-up text-[8px] md:text-[10px]"></i>
                              </button>
                              
                              <button 
                                onClick={(e) => handleFavoriteToggle(item.id, e)}
                                className={`w-6 h-6 md:w-7 md:h-7 rounded-full border transition flex items-center justify-center shadow-sm ${
                                  isFav 
                                  ? 'bg-red-50 border-red-200 text-red-500 hover:bg-red-100' 
                                  : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-600 text-gray-300 hover:text-red-400'
                                }`}
                              >
                                <i className={`${isFav ? 'fas' : 'far'} fa-heart text-[8px] md:text-[10px]`}></i>
                              </button>
