      ...
      <div className="space-y-4 md:space-y-8 animate-fade-in max-w-4xl mx-auto pb-20 px-2 md:px-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
          {currentTitles.map((title, idx) => (
            <button key={idx} onClick={() => handleSelectTitle(title)} className="bg-white dark:bg-gray-800 p-4 md:p-8 rounded-2xl md:rounded-[2.5rem] border border-gray-100 dark:border-gray-700 hover:border-lovelya-400 hover:shadow-2xl transition-all text-left h-full flex flex-col justify-between group transform hover:-translate-y-1 md:hover:-translate-y-2">
              ...
              <div className="w-8 h-8 md:w-12 md:h-12 rounded-full bg-lovelya-50 dark:bg-gray-700 text-lovelya-400 group-hover:bg-lovelya-500 group-hover:text-white transition-all flex items-center justify-center mt-4 md:mt-8 shadow-sm"><i className="fas fa-book-open text-xs md:text-lg"></i></div>
            </button>
          ))}
          {/* Custom Title Card */}
          <button 
            onClick={() => setShowTitleModal(true)} 
            className="bg-gray-50 dark:bg-gray-800/50 p-4 md:p-8 rounded-2xl md:rounded-[2.5rem] border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-lovelya-400 hover:bg-lovelya-50 dark:hover:bg-lovelya-900/20 transition flex flex-col items-center justify-center gap-2 md:gap-4 group min-h-[100px] md:min-h-[180px] transform hover:-translate-y-1 md:hover:-translate-y-2"
          >
            <div className="w-8 h-8 md:w-16 md:h-16 rounded-full bg-white dark:bg-gray-700 flex items-center justify-center shadow-sm group-hover:scale-110 transition text-lovelya-500 text-sm md:text-3xl">
              <i className="fas fa-plus"></i>
            </div>
            <span className="font-bold text-xs md:text-lg text-gray-500 dark:text-gray-400 group-hover:text-lovelya-600">Custom +</span>
          </button>
        </div>
  ...
  const renderReading = () => {
    ...
    return (
      <div className={`transition-all duration-500 ${isFocusMode ? 'fixed inset-0 z-[60] bg-gray-50 dark:bg-gray-900 overflow-y-auto' : 'max-w-4xl mx-auto px-2 md:px-0'}`}>
        ...
        <div className={`bg-white dark:bg-gray-800 transition-all duration-500 ${isFocusMode ? 'min-h-screen py-12 px-6' : 'p-5 md:p-12 rounded-3xl md:rounded-[2.5rem] shadow-xl border border-lovelya-100 dark:border-lovelya-800 mb-32'}`}>
          <div className="max-w-[75ch] mx-auto">
            <h1 className="text-2xl md:text-5xl font-black text-gray-900 dark:text-white mb-5 md:mb-8 pb-5 md:pb-8 border-b border-gray-100 dark:border-gray-700 leading-tight">{content?.title || selectedTitle}</h1>
            
            <div className="prose dark:prose-invert max-w-none leading-[1.7] md:leading-[1.9] font-medium text-gray-700 dark:text-gray-300 space-y-6 md:space-y-10" style={{ fontSize: `${fontSize}px` }}>
              {content ? (
            ...
            {analysisResult && (
              <div id="analysis-results" className="mt-12 md:mt-20 pt-12 md:pt-20 border-t-2 border-gray-100 dark:border-gray-800 animate-slide-up space-y-6 md:space-y-10">
                <div className="flex items-center gap-3 md:gap-4"><div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-lovelya-500 text-white flex items-center justify-center shadow-lg"><i className="fas fa-chart-line text-lg md:text-xl"></i></div><h2 className="text-xl md:text-2xl font-black text-gray-800 dark:text-white uppercase tracking-tight">Summary</h2></div>
                <div className="grid grid-cols-2 md:grid-cols-2 gap-4 md:gap-6">
                   <div className="p-5 md:p-8 bg-gray-50 dark:bg-gray-800/50 rounded-2xl md:rounded-[2rem] text-center"><div className="text-3xl md:text-6xl font-black text-lovelya-600">{Math.round(analysisResult.score)}</div><div className="text-[8px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1 md:mt-2">Score</div></div>
                   <div className="p-5 md:p-8 bg-green-50 dark:bg-green-900/10 rounded-2xl md:rounded-[2rem] text-center"><div className="text-3xl md:text-6xl font-black text-green-600">{Math.round(analysisResult.accuracy)}%</div><div className="text-[8px] md:text-[10px] font-black text-green-500 uppercase tracking-widest mt-1 md:mt-2">Accuracy</div></div>
                </div>
    ...
    <div className="max-w-6xl mx-auto">
      {step === 'setup' && (
        <div className="max-w-2xl mx-auto px-2 md:px-0">
          ...
          <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-soft border border-lovelya-100 animate-fade-in">
            <h2 className="text-xl md:text-3xl font-extrabold mb-5 md:mb-8 text-gray-800 dark:text-white text-center">Reading Practice</h2>
          {loading ? (
             <div className="flex flex-col items-center py-12 md:py-20 animate-pulse">
                <i className="fas fa-magic text-3xl md:text-5xl text-lovelya-500 mb-4 md:mb-6"></i>
                <p className="font-black text-gray-400 uppercase tracking-widest text-[10px] md:text-sm">{statusMsg || 'Building Your Experience...'}</p>
             </div>
          ) : (
          <div className="space-y-5 md:space-y-8">
            <div>
              <label className="block text-[10px] md:text-sm font-bold text-gray-400 mb-2 md:mb-3 uppercase tracking-widest">1. Select Proficiency Level</label>
              <div className="grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-3">
                {LEVELS.map(l => <button key={l} onClick={() => setLevel(l)} className={`py-2 md:py-3 rounded-lg md:rounded-xl font-bold text-xs md:text-base transition-all ${level === l ? 'bg-lovelya-500 text-white shadow-lg' : 'bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>{l}</button>)}
              </div>
            </div>

            <div>
              <label className="block text-[10px] md:text-sm font-bold text-gray-400 mb-2 md:mb-3 uppercase tracking-widest">2. Select Category</label>
              <div className="flex gap-1.5 md:gap-2 p-1 bg-gray-50 dark:bg-gray-700 rounded-xl md:rounded-2xl">
                 {(['islamic', 'general', 'custom'] as const).map(cat => (
                   <button 
                    key={cat}
                    onClick={() => setThemeCategory(cat)}
                    className={`flex-1 py-2 md:py-3 rounded-lg md:rounded-xl font-bold text-xs md:text-base capitalize transition-all ${themeCategory === cat ? 'bg-white dark:bg-gray-600 text-lovelya-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                   >
                     {cat}
                   </button>
                 ))}
            ...
            {themeCategory !== 'custom' ? (
              <div className="animate-fade-in">
                <label className="block text-[10px] md:text-sm font-bold text-gray-400 mb-2 md:mb-3 uppercase tracking-widest">3. Pick a Theme</label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3 max-h-48 md:max-h-60 overflow-y-auto custom-scrollbar pr-1 md:pr-2">
                  {THEMES.filter(t => themeCategory === 'islamic' ? t.isIslamic : !t.isIslamic).map(t => (
                    <button key={t.id} onClick={() => setTheme(t)} className={`p-3 md:p-4 rounded-lg md:rounded-xl text-left transition border-2 text-xs md:text-base ${theme.id === t.id ? 'border-lovelya-500 bg-lovelya-50 dark:bg-lovelya-900/20' : 'border-transparent bg-gray-50 dark:bg-gray-700 hover:border-gray-200'}`}>
              ...
              <div className="space-y-3 md:space-y-4 animate-slide-up">
                <div>
                  <label className="block text-[10px] md:text-sm font-bold text-gray-400 mb-1.5 md:mb-2 uppercase tracking-widest">3. Topic Theme</label>
                  <input value={customTopic} onChange={e => setCustomTopic(e.target.value)} placeholder="e.g. History of Islamic Art..." className="w-full p-3 md:p-4 rounded-xl md:rounded-2xl border-2 border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 outline-none focus:border-lovelya-500 transition text-xs md:text-base" />
                </div>
                <div>
                  <label className="block text-[10px] md:text-sm font-bold text-gray-400 mb-1.5 md:mb-2 uppercase tracking-widest">4. Specific Title (Optional)</label>
                  <input value={customTitle} onChange={e => setCustomTitle(e.target.value)} placeholder="Type a title or leave empty for AI suggestions..." className="w-full p-3 md:p-4 rounded-xl md:rounded-2xl border-2 border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 outline-none focus:border-lovelya-500 transition text-xs md:text-base" />
                </div>
              </div>
            )}
            {error && <div className="p-3 md:p-4 bg-red-50 text-red-500 rounded-xl text-[10px] md:text-sm font-bold text-center border border-red-100">{error}</div>}
            <button onClick={() => handleStartSetup(false)} disabled={loading || (themeCategory === 'islamic' && !theme) || (themeCategory === 'custom' && !customTopic)} className="w-full py-3.5 md:py-5 mt-4 md:mt-6 rounded-xl md:rounded-2xl bg-lovelya-600 text-white font-black text-sm md:text-lg shadow-xl hover:bg-lovelya-700 transition disabled:opacity-50">Generate Content</button>
          </div>
          )}
        </div>
        </div>
