        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
          {bridgeEntries.map(bridge => (
            <button 
              ...
              className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-gray-100 dark:border-gray-700 text-left hover:shadow-xl hover:-translate-y-1 transition-all group flex flex-col h-full"
            >
              <div className="w-9 h-9 md:w-12 md:h-12 rounded-xl bg-purple-50 dark:bg-purple-900/30 text-purple-600 flex items-center justify-center mb-3 md:mb-4 group-hover:scale-110 transition shrink-0">
                <i className="fas fa-bookmark text-xs md:text-base"></i>
              </div>
              ...
              <div className="mt-auto pt-2 md:pt-4 border-t border-gray-50 dark:border-gray-700 flex justify-between items-center text-[7px] md:text-[10px] font-black uppercase tracking-wider text-purple-500">
                <span className="hidden md:inline">Read Insight</span> <i className="fas fa-arrow-right"></i>
              </div>
            </button>
          ))}
  ...
  return (
    <div className="max-w-6xl mx-auto pb-24 md:pb-32 animate-fade-in" ref={scrollContainerRef}>
      ...
      <div className="text-center mb-6 md:mb-12 px-4">
        ...
        <h2 className="text-2xl md:text-5xl font-black text-gray-900 dark:text-white mb-2 md:mb-4 tracking-tight">Curriculum Roadmap</h2>
        <p className="text-gray-500 dark:text-gray-400 max-w-2xl mx-auto font-medium text-[11px] md:text-lg leading-relaxed">
      ...
      <div className="sticky top-4 z-[40] px-4 mb-8 md:mb-12">
        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl p-1.5 md:p-2 rounded-2xl md:rounded-[2rem] shadow-xl border border-lovelya-100 dark:border-gray-700 flex gap-1 overflow-x-auto no-scrollbar">
          <button 
            ...
            <section key={levelData.level} className="animate-slide-up">
                ...
                {levelData.units.map((unit) => {
                    ...
                    <button 
                        ...
                        className="group relative bg-white dark:bg-gray-800 p-3 md:p-8 rounded-2xl md:rounded-[3rem] border border-gray-100 dark:border-gray-700 text-left hover:border-lovelya-400 hover:shadow-2xl transition-all duration-500 flex flex-col h-full overflow-hidden"
                    >
                        ...
                        <div className="flex justify-between items-start mb-2 md:mb-6">
                            <div className={`w-6 h-6 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center text-[10px] md:text-xl shadow-sm shrink-0 ${completed ? 'bg-green-500 text-white' : 'bg-lovelya-50 dark:bg-gray-700 text-lovelya-600'}`}>
                                {completed ? <i className="fas fa-check"></i> : <span className="font-black italic text-[10px] md:text-xl">{unit.unitNumber}</span>}
                            </div>
                            {completed ? (
                                <span className="bg-green-100 text-green-700 px-1.5 py-0.5 md:px-3 md:py-1 rounded-full text-[6px] md:text-[10px] font-black uppercase tracking-widest">Mastered</span>
                            ) : stepsCompleted > 0 ? (
                                <span className="bg-lovelya-50 text-lovelya-600 px-1.5 py-0.5 md:px-3 md:py-1 rounded-full text-[6px] md:text-[10px] font-black uppercase tracking-widest">In Progress</span>
                            ) : (
                                <span className="bg-gray-100 dark:bg-gray-700 text-gray-500 px-1.5 py-0.5 md:px-3 md:py-1 rounded-full text-[6px] md:text-[10px] font-black uppercase tracking-widest">Available</span>
                            )}
                        </div>

                        <h4 className="text-[11px] md:text-2xl font-black text-gray-800 dark:text-white mb-1 md:mb-3 leading-tight group-hover:text-lovelya-600 transition-colors line-clamp-2">
                            {unit.title}
                        </h4>
                        <p className="text-[9px] md:text-sm text-gray-500 dark:text-gray-400 leading-tight md:leading-relaxed mb-3 md:mb-8 flex-1 line-clamp-2 md:line-clamp-3">
                            {unit.description}
                        </p>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-12">
                   <div className="max-w-[70ch] mx-auto space-y-8 md:space-y-12">
                      ...
                      <section className="bg-gray-50 dark:bg-gray-900/50 p-6 md:p-8 rounded-3xl border-l-4 border-purple-400">
                         <p className="text-lg md:text-xl font-serif italic text-gray-700 dark:text-gray-300 leading-relaxed">
                      ...
                      <section className="space-y-4 md:space-y-6">
                         ...
                         <p className="text-base md:text-lg text-gray-600 dark:text-gray-300 leading-[1.7] md:leading-[1.8] font-medium">
                      ...
                      <section className="bg-lovelya-50 dark:bg-lovelya-900/10 p-6 md:p-8 rounded-3xl border border-lovelya-100 dark:border-lovelya-900">
                         <h3 className="text-lg md:text-xl font-black text-lovelya-700 dark:text-lovelya-400 mb-4 uppercase tracking-widest flex items-center gap-2">
                           <i className="fas fa-link"></i> Grammar Connection
                         </h3>
                         <p className="text-base md:text-lg text-lovelya-800 dark:text-lovelya-200 leading-relaxed font-bold">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                           {activeBridge.cefrFocus && (
                              <section className="bg-gray-50 dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700">
                                 <h3 className="text-sm font-black text-gray-700 dark:text-gray-400 mb-3 uppercase tracking-widest flex items-center gap-2">
                           ...
                           {activeBridge.proTips && activeBridge.proTips.length > 0 && (
                              <section className="bg-indigo-50 dark:bg-indigo-900/10 p-6 rounded-3xl border border-indigo-100 dark:border-indigo-900/30">
                                 <h3 className="text-sm font-black text-indigo-700 dark:text-indigo-400 mb-3 uppercase tracking-widest flex items-center gap-2">
                      ...
                      {activeBridge.detailedExplanation && (
                        <section className="bg-gray-50 dark:bg-gray-800 p-6 md:p-8 rounded-3xl border border-gray-100 dark:border-gray-700">
                           <h3 className="text-lg md:text-xl font-black text-gray-700 dark:text-gray-400 mb-4 uppercase tracking-widest flex items-center gap-2">
                              <i className="fas fa-book-open"></i> Mastery Insights: Core Concepts & Islamic Examples
                           </h3>
                           <div className="prose dark:prose-invert max-w-none text-gray-900 dark:text-gray-100 whitespace-pre-line leading-relaxed">
                              {activeBridge.detailedExplanation}
                           </div>
                        </section>
                      )}
                            ...
                            <div className="p-5 md:p-6 space-y-4">
                               {activeBridge.scenarioDialogue.map((line, idx) => (
                                 <div key={idx} className="flex flex-col sm:flex-row gap-1 sm:gap-4">
                                    <span className="font-black text-lovelya-600 dark:text-lovelya-400 min-w-[80px] text-[11px] md:text-sm uppercase">{line.speaker}:</span>
                                    <span className="text-gray-700 dark:text-gray-300 text-base md:text-lg">{line.text}</span>
                                 </div>
                               ))}
                            </div>
                         </div>
