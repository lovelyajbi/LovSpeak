...
const AreaChart = ({ data, metric, moduleId }: { data: ChartDataPoint[], metric: MetricType, moduleId: string }) => {
    ...
    const paddingX = 40; // Increased padding for better label fit
    const paddingY = 30;
    
    const containerWidth = typeof window !== 'undefined' ? window.innerWidth : 1200;
    const minPointWidth = isMobile ? 50 : 90; // Slightly wider for better spacing
    
    // Ensure we show at least 12 points for daily or enough to fill screen
    ...
    const getCurve = (i: number, data: ChartDataPoint[]) => {
        const x = getX(i);
        ...
        // Use normalized control points for a more fluid curve
...
const ProfileModule: React.FC = () => {
  const { signout, user: authUser, isSyncing } = useAuth();
  ...
  const generateTrendData = (filter: TimeFilter, metric: MetricType, specificModule: string | null = null): ChartDataPoint[] => {
    ...
    if (filter === 'today') {
        ...
        // Updated to show last 12 days instead of 7 to fulfill "minimal 12 points" requirement
        for (let i = 11; i >= 0; i--) {
            const d = new Date(now);
            d.setDate(d.getDate() - i);
            dataPoints.push(generatePoint(d.toDateString(), d.toLocaleDateString('en-US', { weekday: 'short', day: 'numeric' })));
        }
        filtered.forEach(log => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 animate-fade-in pb-10">
        ...
        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-2xl rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 shadow-xl border border-white/40 dark:border-gray-700 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-5 md:gap-7 items-center relative z-10">
                ...
                <div className="md:col-span-4 flex flex-col items-center md:items-start text-center md:text-left">
                    ...
                    <div className="space-y-0.5 w-full">
                        <input 
                            ...
                            className="text-lg md:text-xl font-black text-gray-900 dark:text-white bg-transparent border-b-2 border-transparent hover:border-gray-200 focus:border-lovelya-500 outline-none transition w-full text-center md:text-left py-0.5 truncate"
                        />
                        <div className="inline-flex items-center gap-1.5 bg-lovelya-50 dark:bg-lovelya-500/10 px-2 py-0.5 rounded-full border border-lovelya-100 dark:border-lovelya-500/20">
                            <span className="text-lovelya-600 dark:text-lovelya-400 text-[8px] md:text-[9px] font-black uppercase tracking-widest">{currentLvl.level}</span>
                            <span className="w-0.5 h-0.5 rounded-full bg-lovelya-300 dark:bg-lovelya-700"></span>
                            <span className="text-gray-700 dark:text-gray-200 text-[8px] md:text-[9px] font-bold">{currentLvl.title}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="md:col-span-8 space-y-4">
                    {/* Progress Bar */}
                    <div className="bg-white dark:bg-gray-900/40 p-3 rounded-xl border border-gray-100 dark:border-gray-700/50 shadow-sm">
                        <div className="flex justify-between items-center mb-1.5 text-[8px] md:text-[10px] font-black">
                        ...
                        <div className="overflow-hidden h-1.5 md:h-2 text-xs flex rounded-full bg-gray-100 dark:bg-gray-800">
                            <div 
                                style={{ width: `${xpProgress}%` }} 
                                className="shadow-[0_0_10px_rgba(236,72,153,0.2)] flex flex-col text-center whitespace-nowrap text-white justify-center bg-gradient-to-r from-lovelya-400 via-fuchsia-500 to-purple-600 transition-all duration-1000 ease-out"
                            ></div>
                        </div>
                    </div>
        ...
        {/* --- WEEKLY AI INSIGHT --- */}
        <div className="relative overflow-hidden rounded-[1.5rem] md:rounded-[2rem] bg-white dark:bg-gray-800 shadow-lg border border-gray-100 dark:border-gray-700 p-6 md:p-8 group">
                ...
                <div className="p-4 bg-indigo-50/50 dark:bg-indigo-900/10 rounded-2xl border border-indigo-100 dark:border-indigo-800/50 min-h-[80px] flex items-center justify-center">
                    {isLoadingInsight ? (
                        ...
                        <p className="text-xs text-gray-700 dark:text-gray-300 font-medium leading-relaxed italic">
                            {aiInsight || `"Keep going, ${profile.name}! Every small step in learning English is a step closer to success. Consistency is beloved by Allah, even if it's a little."`}
                        </p>
                    )}
                </div>
            </div>
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-[2rem] p-5 md:p-8 shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-3">
                    <div>
                        <h4 className="font-black text-sm md:text-base text-gray-800 dark:text-white uppercase tracking-tight">Skill Comparison</h4>
            ...
            <div className="h-[250px] md:h-[400px] relative">
                {chartType === 'bar' ? (
                        <div className="h-full flex items-end justify-around md:justify-between gap-1.5 md:gap-4 px-1">
                            {moduleStats.map((stat) => {
                                ...
                                return (
                                    <div key={stat.module} className="flex-1 flex flex-col items-center group relative h-full justify-end max-w-[60px] md:max-w-[80px]">
                                        ...
                                        <div className="w-full bg-gray-100 dark:bg-gray-700/50 rounded-xl md:rounded-2xl relative overflow-hidden flex items-end h-[160px] md:h-[300px] border border-transparent group-hover:border-gray-200 dark:group-hover:border-gray-600 transition-all">
                                            <div 
                                        ...
                                        <div className="mt-2 text-[8px] md:text-[10px] font-black text-gray-400 uppercase tracking-tighter md:tracking-widest text-center h-8 md:h-10 flex items-center justify-center w-full break-words leading-none group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors">
                                            {label}
                                        </div>
                                    </div>
                                );
