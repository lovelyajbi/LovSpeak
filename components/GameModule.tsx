    ...
    <div className="max-w-4xl mx-auto min-h-[70vh] relative">
      ...
      {gameState === 'levels' && selectedCategory && selectedContext && (
          <div className="animate-fade-in py-6 md:py-10 text-center">
              ...
              <p className="text-xs md:text-base text-gray-500 mb-6 md:mb-8 font-medium capitalize">{selectedContext} Track Progression</p>
              
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-3 md:gap-4 max-w-3xl mx-auto px-4">
                  {[...Array(TOTAL_LEVELS)].slice((currentLevelPage - 1) * levelsPerPage, currentLevelPage * levelsPerPage).map((_, i) => {
                      ...
                      return (
                          <button 
                            ...
                            className={`aspect-square relative flex flex-col items-center justify-center rounded-2xl md:rounded-[2rem] transition-all shadow-sm group overflow-hidden
                          ...
                          >
                              {isLocked ? (
                                  ...
                                  <>
                                      <span className={`text-xl md:text-3xl font-black ${isCompleted ? 'text-lovelya-500' : 'text-gray-800 dark:text-white'}`}>{levelNum}</span>
                                      {isCompleted && (
                  ...
                  {feedback ? (
                    ...
                    <div className="w-full max-w-2xl animate-fade-in space-y-8">
                        ...
                        {selectedCategory === 'knowledge' && (
                            <>
                                <div className="p-5 md:p-8 bg-rose-50 dark:bg-rose-900/10 rounded-2xl md:rounded-[3rem] text-lg md:text-2xl font-bold text-rose-900 dark:text-rose-200 border border-rose-100 dark:border-rose-800">{items[currentRound].question}</div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 md:gap-4 w-full">
                                    {items[currentRound].options.map((opt: string, idx: number) => (
                                        <button key={idx} onClick={() => handleAnswer(idx === items[currentRound].correctIndex)} className="p-3.5 md:p-5 rounded-xl md:rounded-2xl font-bold border-2 border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-all text-sm md:text-lg">{opt}</button>
                                    ))}
                                </div>
                            </>
                        )}
                        ...
                        {selectedCategory === 'grammar_strike' && (
                            <>
                                <div className="p-5 md:p-8 bg-red-50 dark:bg-red-900/10 rounded-2xl md:rounded-[3rem] border border-red-100 dark:border-red-900">
                                    ...
                                    <p className="text-lg md:text-2xl font-bold text-gray-800 dark:text-white italic">"{items[currentRound].sentence_with_error}"</p>
                                </div>
                                <input autoFocus value={userInput} onChange={e => setUserInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAnswer(isAnswerCorrect(userInput, items[currentRound].correction))} className="w-full p-3.5 md:p-5 text-center text-lg md:text-2xl font-bold rounded-2xl md:rounded-3xl border-4 border-red-100 focus:border-red-500 outline-none transition-all dark:bg-gray-900" placeholder="Type the correct sentence..." />
                            </>
                        )}
