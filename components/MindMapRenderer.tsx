...
const NodeItem: React.FC<{ node: MindMapNode; isRoot?: boolean }> = ({ node, isRoot = false }) => {
  ...
  let detailClass = "text-xs mt-1 text-gray-500 dark:text-gray-400";
  let icon = "";

  switch (node.type) {
    case 'root':
      containerClass += " bg-gradient-to-br from-lovelya-500 to-purple-600 text-white shadow-xl border-transparent scale-105 mb-6";
      labelClass = "text-2xl font-black text-white tracking-tight";
      detailClass = "text-sm text-white/80 font-medium";
      icon = "fa-brain";
      break;
    case 'main':
      ...
      labelClass = "text-lg text-lovelya-700 dark:text-lovelya-300";
