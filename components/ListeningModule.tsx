...
const ListeningModule: React.FC<ModuleProps> = ({ onComplete, initialContext, onNavigate }) => {
  ...
  const handleSubmitQuiz = () => {
    ...
    if (initialContext?.stepId && finalScore >= 60) {
        completeRoadmapUnit(initialContext.stepId);
    }

    setStep('result');
    if (onComplete && finalScore >= 60) onComplete();
  };
  ...
  const renderSetup = () => (
    <div className="max-w-4xl mx-auto px-2 md:px-0">
      ...
      <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-sm border border-lovelya-100 dark:border-gray-700 animate-fade-in">
        <h2 className="text-xl md:text-3xl font-bold mb-5 md:mb-6 text-lovelya-700 dark:text-lovelya-300 text-center">
      ...
      <div className="flex justify-center mb-6 md:mb-8">
        <div className="bg-gray-100 dark:bg-gray-700 p-1 rounded-xl flex">
           <button onClick={() => setIsCustomMode(false)} className={`px-4 py-1.5 md:px-6 md:py-2 rounded-lg text-xs md:text-sm font-bold transition ${!isCustomMode ? 'bg-white dark:bg-gray-600 shadow text-lovelya-600 dark:text-white' : 'text-gray-500'}`}>Standard Topics</button>
           <button onClick={() => setIsCustomMode(true)} className={`px-4 py-1.5 md:px-6 md:py-2 rounded-lg text-xs md:text-sm font-bold transition ${isCustomMode ? 'bg-white dark:bg-gray-600 shadow text-lovelya-600 dark:text-white' : 'text-gray-500'}`}>Custom Topic</button>
        </div>
      </div>

      <div className="space-y-5 md:space-y-6">
        <div>
          <label className="block text-sm md:text-lg font-bold text-gray-700 dark:text-gray-300 mb-2 md:mb-3">1. Select Level</label>
          <div className="grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-3">
            {LEVELS.map(l => (
              <button key={l} onClick={() => setLevel(l)} className={`py-2 md:py-3 rounded-lg md:rounded-xl text-xs md:text-base font-bold transition-all ${level === l ? 'bg-lovelya-500 text-white shadow-lg' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200'}`}>{l}</button>
            ))}
          </div>
        </div>

        <div>
           <label className="block text-sm md:text-lg font-bold text-gray-700 dark:text-gray-300 mb-2 md:mb-3">2. Select Type</label>
           <div className="flex gap-2 md:gap-4">
             <button onClick={() => setType('monologue')} className={`flex-1 p-3 md:p-4 rounded-xl border-2 transition flex items-center justify-center gap-2 md:gap-3 text-xs md:text-base ${type === 'monologue' ? 'border-lovelya-500 bg-lovelya-50 dark:bg-lovelya-900/20 text-lovelya-700 font-bold' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300'}`}><i className="fas fa-user text-base md:text-xl"></i> Monologue</button>
             <button onClick={() => setType('dialogue')} className={`flex-1 p-3 md:p-4 rounded-xl border-2 transition flex items-center justify-center gap-2 md:gap-3 text-xs md:text-base ${type === 'dialogue' ? 'border-lovelya-500 bg-lovelya-50 dark:bg-lovelya-900/20 text-lovelya-700 font-bold' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300'}`}><i className="fas fa-user-friends text-base md:text-xl"></i> Dialogue</button>
           </div>
        </div>

        {!isCustomMode ? (
          <div>
            <label className="block text-sm md:text-lg font-bold text-gray-700 dark:text-gray-300 mb-2 md:mb-3">3. Choose Topic</label>
            <div className="flex gap-2 md:gap-4 mb-3 md:mb-4">
              <button onClick={() => setThemeCategory('islamic')} className={`flex-1 py-2 md:py-2.5 rounded-lg md:rounded-xl border-2 text-[10px] md:text-sm font-bold transition flex items-center justify-center gap-1.5 md:gap-2 ${themeCategory === 'islamic' ? 'border-lovelya-500 text-lovelya-600 bg-lovelya-50 dark:bg-lovelya-900/20' : 'border-gray-200 text-gray-500 dark:border-gray-700 dark:text-gray-400'}`}><i className="fas fa-moon"></i> Islamic</button>
              <button onClick={() => setThemeCategory('general')} className={`flex-1 py-2 md:py-2.5 rounded-lg md:rounded-xl border-2 text-[10px] md:text-sm font-bold transition flex items-center justify-center gap-1.5 md:gap-2 ${themeCategory === 'general' ? 'border-lovelya-500 text-lovelya-600 bg-lovelya-50 dark:bg-lovelya-900/20' : 'border-gray-200 text-gray-500 dark:border-gray-700 dark:text-gray-400'}`}><i className="fas fa-globe"></i> General</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-3 max-h-40 md:max-h-60 overflow-y-auto custom-scrollbar p-1">
              {LISTENING_THEMES.filter(t => themeCategory === 'islamic' ? t.isIslamic : !t.isIslamic).map(t => (
                <button key={t.id} onClick={() => setThemeId(t.id)} className={`p-3 md:p-4 rounded-xl text-left transition border-2 text-[11px] md:text-sm ${themeId === t.id ? 'border-lovelya-500 bg-lovelya-50 dark:bg-lovelya-900/20' : 'border-transparent bg-gray-50 dark:bg-gray-700 hover:border-gray-300'}`}><div className="font-bold text-gray-800 dark:text-white flex justify-between items-center">{t.name}{t.isIslamic && <span title="Islamic Content">🌙</span>}</div></button>
              ))}
            </div>
          </div>
        ) : (
          <div className="space-y-3 md:space-y-4 animate-slide-up">
             <div><label className="block text-sm md:text-lg font-bold text-gray-700 dark:text-gray-300 mb-1.5 md:mb-2">3. Topic Theme</label><input value={customTopic} onChange={e => setCustomTopic(e.target.value)} placeholder="e.g. Life in Medina, Ancient Civilization..." className="w-full p-3 md:p-4 rounded-xl border-2 border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 focus:ring-2 focus:ring-lovelya-500 outline-none text-xs md:text-base" /></div>
             <div><label className="block text-sm md:text-lg font-bold text-gray-700 dark:text-gray-300 mb-1.5 md:mb-2">4. Specific Title <span className="text-[10px] md:text-sm font-normal text-gray-500 ml-1">(Optional)</span></label><input value={customTitle} onChange={e => setCustomTitle(e.target.value)} placeholder="Leave empty for AI suggestions..." className="w-full p-3 md:p-4 rounded-xl border-2 border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 focus:ring-2 focus:ring-lovelya-500 outline-none text-xs md:text-base" /></div>
          </div>
        )}
        ...
        <button onClick={() => !isCustomMode ? handleFetchTitles(false) : handleCustomStart()} disabled={loading || (!isCustomMode && !themeId) || (isCustomMode && !customTopic)} className="w-full py-3.5 md:py-4 rounded-xl bg-lovelya-600 text-white font-black text-sm md:text-lg shadow-lg hover:bg-lovelya-700 transition disabled:opacity-50 flex items-center justify-center gap-2 md:gap-3">
          {loading ? <span className="flex items-center gap-2"><i className="fas fa-spinner fa-spin"></i> {statusMsg || 'Loading...'}</span> : <span className="flex items-center gap-2">{!isCustomMode ? <i className="fas fa-search"></i> : <i className="fas fa-magic"></i>}{!isCustomMode ? 'Find Audio Content' : (customTitle ? 'Generate Custom Audio' : 'Generate Suggestions')}</span>}
        </button>
      </div>
    </div>
    </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5">
              {currentTitles.map((title, idx) => (
                <button key={idx} onClick={() => handleSelectTitle(title)} className="bg-white dark:bg-gray-800 p-4 md:p-8 rounded-2xl md:rounded-[2rem] border border-gray-100 dark:border-gray-700 hover:border-lovelya-400 hover:shadow-2xl transition-all text-left h-full flex flex-col justify-between group transform hover:-translate-y-1 md:hover:-translate-y-2">
                  <h3 className="font-bold text-sm md:text-xl text-gray-800 dark:text-gray-100 group-hover:text-lovelya-600 leading-tight line-clamp-2 md:line-clamp-3">{title}</h3>
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-lovelya-50 dark:bg-gray-700 text-lovelya-400 group-hover:bg-lovelya-500 group-hover:text-white transition-all flex items-center justify-center mt-3 md:mt-6"><i className="fas fa-play text-xs md:text-sm"></i></div>
                </button>
              ))}
              {/* Custom Title Card */}
              <button 
                onClick={() => setShowTitleModal(true)} 
                className="bg-gray-50 dark:bg-gray-800/50 p-4 md:p-8 rounded-2xl md:rounded-[2rem] border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-lovelya-400 hover:bg-lovelya-50 dark:hover:bg-lovelya-900/20 transition flex flex-col items-center justify-center gap-2 md:gap-4 group min-h-[100px] md:min-h-[160px] transform hover:-translate-y-1 md:hover:-translate-y-2"
              >
                <div className="w-8 h-8 md:w-14 md:h-14 rounded-full bg-white dark:bg-gray-700 flex items-center justify-center shadow-sm group-hover:scale-110 transition text-lovelya-500 text-base md:text-2xl">
                  <i className="fas fa-plus"></i>
                </div>
                <span className="font-bold text-[10px] md:text-base text-gray-500 dark:text-gray-400 group-hover:text-lovelya-600">Custom +</span>
              </button>
            </div>
  ...
  const renderPlayer = () => (
    <div className="max-w-3xl mx-auto animate-fade-in space-y-4 md:space-y-6 px-2 md:px-0">
       ...
       <div className="bg-white dark:bg-gray-800 p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-lg border border-gray-100 dark:border-gray-700 text-center relative">
