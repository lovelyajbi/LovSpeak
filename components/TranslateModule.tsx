...
import { translateText, TranslationResult } from '../services/gemini';
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-lovelya-100 dark:border-gray-700 overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100 dark:divide-gray-600">
          ...
          <div className="relative w-full h-auto min-h-40 md:min-h-64 flex flex-col">
            <div className="flex-1 p-4 md:p-6 pr-12 bg-gray-50 dark:bg-gray-800/50 text-gray-900 dark:text-gray-200 text-base md:text-lg min-h-[10rem]">
                {loading ? (
                    <div className="h-full flex items-center justify-center text-lovelya-400">
                        <i className="fas fa-spinner fa-spin text-2xl"></i>
                    </div>
                ) : result?.translation ? (
                    <div onMouseUp={handleTextSelect} className="whitespace-pre-wrap">
                        {result.translation}
                    </div>
                ) : (
                    <span className="text-gray-400 dark:text-gray-500 italic">Translation will appear here...</span>
                )}
            </div>
            ...
            {/* Synonyms and Examples Sections */}
            <AnimatePresence>
                {result && !loading && (
                    <motion.div 
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="p-4 md:p-6 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 space-y-4"
                    >
                        {result.synonyms && result.synonyms.length > 0 && (
                            <div>
                                <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Synonyms</h4>
                                <div className="flex flex-wrap gap-2">
                                    {result.synonyms.map((s, i) => (
                                        <span key={i} className="px-3 py-1 bg-lovelya-50 dark:bg-gray-700 text-lovelya-700 dark:text-lovelya-300 rounded-full text-xs font-bold border border-lovelya-100 dark:border-gray-600">
                                            {s}
                                        </span>
                                    ))}
                                </div>
                        ...
                        {result.examples && result.examples.length > 0 && (
                            <div>
                                <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Example Sentences</h4>
                                <ul className="space-y-2">
                                    {result.examples.map((ex, i) => (
                                        <li key={i} className="text-xs md:text-sm text-gray-600 dark:text-gray-400 italic flex gap-2">
                                            <i className="fas fa-quote-left text-lovelya-300 text-[10px] mt-1"></i>
                                            {ex}
                                        </li>
                                    ))}
                                </ul>
