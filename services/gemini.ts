...
const getAiClient = () => {
  ...
  if (!apiKey) {
    throw new Error("AI API Key not found. Please set it in your profile.");
  }
...
export const generateGrammarTask = async (lessonTitle: string): Promise<string> => {
    try {
        const ai = getAiClient();
        const prompt = `Create a specific English writing task for the grammar topic: "${lessonTitle}". 1-2 sentences of instruction. ${getLanguageInstruction()} ${STRICT_FILTER}`;
        const response = await ai.models.generateContent({
            model: 'gemini-3-flash-preview',
            contents: prompt,
            config: {
...
export const translateText = async (text: string, direction: 'en-id' | 'id-en'): Promise<string> => {
    try {
        const ai = getAiClient();
        const prompt = direction === 'en-id' ? `Translate to Indonesian: "${text}"` : `Translate to English: "${text}"`;
        const response = await ai.models.generateContent({
            model: 'gemini-3-flash-preview',
            contents: prompt,
            config: {
